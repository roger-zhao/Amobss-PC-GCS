...
namespace MissionPlanner
{
    public class CurrentState : ICloneable
    {
        ...
        enum PID_TUNING_AXIS
        {
           ...
           PID_TUNING_VELX, /*  | */
           PID_TUNING_VELY, /*  | */
           PID_TUNING_ROLL_ANGLE, /*  | */
           PID_TUNING_PITCH_ANGLE, /*  | */
           PID_TUNING_YAW_ANGLE, /*  | */
           PID_TUNING_ROLL_RATE, /*  | */
           PID_TUNING_PITCH_RATE, /*  | */
           PID_TUNING_YAW_RATE, /*  | */
           PID_TUNING_ACCZ, /*  | */
           PID_TUNING_STEER, /*  | */
           PID_TUNING_LANDING, /*  | */
        ...
        [DisplayText("Copter type")]
        public string copter_type { get; set; }
            ...
            MAVLinkInterface mavinterface, MAVState MAV)
        {
            lock (this)
            {
                if (DateTime.Now > lastupdate.AddMilliseconds(50) || updatenow) // 20 hz
                {
                    ...
                    //Added by HL below
                    //mavLinkMessage = MAV.getPacket((uint)MAVLink.MAVLINK_MSG_ID.DOSE_INFORMATION);

                    //if (mavLinkMessage != null)
                    //{
                    //    var dose = mavLinkMessage.ToStructure<MAVLink.mavlink_dose_information_t>();

                    //    dose_information = dose.dose_information;
                    //}
                    //Added by HL above
                        ...
                        else
                        {
                            ...
                            if ((hb.base_mode & (byte) MAVLink.MAV_MODE_FLAG.CUSTOM_MODE_ENABLED) != 0)
                            {
                                ...
                                if (_mode != hb.custom_mode)
                                {
                                    ...
                                    if (hb.custom_mode.Equals(0x2F) || hb.custom_mode.Equals(0x3F) || hb.custom_mode.Equals(0x4F))
                                    {
                                        if (hb.custom_mode.Equals(0x2F))
                                        {
                                            mode = "semi-forward";
                                            _mode = hb.custom_mode;
                                        }
                                        else if (hb.custom_mode.Equals(0x3F))
                                        {
                                            mode = "semi-backward";
                                            _mode = hb.custom_mode;
                                        }
                                        else if (hb.custom_mode.Equals(0x4F))
                                        {
                                            mode = "ab-point";
                                            _mode = hb.custom_mode;
                                        }
                                    }
                                    else if(hb.custom_mode.Equals(22))
                                    {
                                        mode = "follow";
                                        _mode = hb.custom_mode;
                                    }
                                    //if (!hb.custom_mode.Equals(0x2F) && !hb.custom_mode.Equals(0x3F))
                                    else
                                    {
                                ...
                                Settings.Instance.GetBoolean("speechmodeenabled"))
                            {
                                ...
                                {
                                    MainV2.speechEngine.SpeakAsync(Common.speechConversion("模式切换到 " + mode_str));
                                    MainV2.lblog.Log(Level.Info, "模式切换到 " + mode_str);
                                }
                    ...
                    if (mavLinkMessage != null)
                    {
                        ...
                        M1 = servoout.servo1_raw;
                        M2 = servoout.servo2_raw;
                        M3 = servoout.servo3_raw;
                        M4 = servoout.servo4_raw;
                        M5 = servoout.servo5_raw;
                        M6 = servoout.servo6_raw;
                        M7 = servoout.servo7_raw;
                        M8 = servoout.servo8_raw;
                    ...
                    if (mavLinkMessage != null)
                    {
                        ...
                        pidaxis = pid.axis;
                        switch ((PID_TUNING_AXIS)pidaxis)
                        {
                            case PID_TUNING_AXIS.PID_TUNING_POSX:
                                PosXTar = pid.desired;
                                PosXAct = pid.achieved;
                                break;
                            case PID_TUNING_AXIS.PID_TUNING_POSY:
                                break;
                            case PID_TUNING_AXIS.PID_TUNING_POSZ:
                                VelZTar = pid.FF; //  borrowed
                                VelZAct = pid.P; //  borrowed
                                PosZTar = pid.desired;
                                PosZAct = pid.achieved;
                                PosXTar = pid.I; //  borrowed 
                                PosXAct = pid.D; //  borrowed 
                                break;
                            case PID_TUNING_AXIS.PID_TUNING_VELX:
                                VelXpidP = pid.P;
                                VelXpidI = pid.I;
                                VelXpidD = pid.D;
                                VelXTar = pid.desired;
                                VelXAct = pid.achieved;
                                break;
                            case PID_TUNING_AXIS.PID_TUNING_VELY:
                                VelYpidP = pid.P;
                                VelYpidI = pid.I;
                                VelYpidD = pid.D;
                                VelYTar = pid.desired;
                                VelYAct = pid.achieved;
                                break;
                            case PID_TUNING_AXIS.PID_TUNING_ROLL_ANGLE:
                                AngYawTar = pid.I;
                                AngYawAct = pid.D;
                                AngPitchTar = pid.FF;
                                AngPitchAct = pid.P;
                                AngRollTar = pid.desired;
                                AngRollAct = pid.achieved;
                                break;
                            case PID_TUNING_AXIS.PID_TUNING_PITCH_ANGLE:
                                AngPitchTar = pid.desired;
                                AngPitchAct = pid.achieved;
                                break;
                            case PID_TUNING_AXIS.PID_TUNING_YAW_ANGLE:
                                AngYawTar = pid.desired;
                                AngYawAct = pid.achieved;
                                break;
                            case PID_TUNING_AXIS.PID_TUNING_ROLL_RATE:
                                RatRollpidP = pid.P;
                                RatRollpidI = pid.I;
                                RatRollpidD = pid.D;
                                RatRollpidff = pid.FF;
                                RatRollTar = rad2deg* pid.desired;
                                RatRollAct = pid.achieved * 0.01f;
                                break;
                            case PID_TUNING_AXIS.PID_TUNING_PITCH_RATE:
                                RatPitchpidP = pid.P;
                                RatPitchpidI = pid.I;
                                RatPitchpidD = pid.D;
                                RatPitchpidff = pid.FF;
                                RatPitchTar = rad2deg * pid.desired;
                                RatPitchAct = pid.achieved * 0.01f;
                                break;
                            case PID_TUNING_AXIS.PID_TUNING_YAW_RATE:
                                RatYawpidP = pid.P;
                                RatYawpidI = pid.I;
                                RatYawpidD = pid.D;
                                RatYawpidff = pid.FF;
                                RatYawTar = rad2deg * pid.desired;
                                RatYawAct = pid.achieved * 0.01f;
                                break;
                            case PID_TUNING_AXIS.PID_TUNING_ACCZ:
                                AccZpidP = pid.P;
                                AccZpidI = pid.I;
                                AccZpidD = pid.D;
                                AccZpidff = pid.FF;
                                AccZTar = pid.desired;
                                AccZAct = pid.achieved;
                                break;
                            default:
                                break;
                        }
        ...
        public float PosXTar { get; set; }

        public float PosXAct { get; set; }

        // PosZ                       
        public float PosZTar { get; set; }
                        
        public float PosZAct { get; set; }

        // VelX
        public float VelXpidP { get; set; }

        public float VelXpidI { get; set; }

        public float VelXpidD { get; set; }

        public float VelXTar { get; set; }

        public float VelXAct { get; set; }

        // VelY
        public float VelYpidP { get; set; }

        public float VelYpidI { get; set; }

        public float VelYpidD { get; set; }

        public float VelYTar { get; set; }

        public float VelYAct { get; set; }

        // VelZ                
        public float VelZTar { get; set; }
                        
        public float VelZAct { get; set; }

        // ACCZ
        public float AccZpidP { get; set; }

        public float AccZpidI { get; set; }
                     
        public float AccZpidD { get; set; }

        public float AccZpidff { get; set; }

        public float AccZTar { get; set; }
                     
        public float AccZAct { get; set; }



        // Angle Roll

        public float AngRollTar { get; set; }

        public float AngRollAct { get; set; }

        // Angle Pitch

        public float AngPitchTar { get; set; }

        public float AngPitchAct { get; set; }

        // Angle Yaw

        public float AngYawTar { get; set; }

        public float AngYawAct { get; set; }

        // Rate Roll
        public float RatRollpidff { get; set; }

        public float RatRollpidP { get; set; }

        public float RatRollpidI { get; set; }

        public float RatRollpidD { get; set; }

        public float RatRollTar { get; set; }

        public float RatRollAct { get; set; }

        // Rate Pitch
        public float RatPitchpidff { get; set; }

        public float RatPitchpidP { get; set; }

        public float RatPitchpidI { get; set; }

        public float RatPitchpidD { get; set; }

        public float RatPitchTar { get; set; }

        public float RatPitchAct { get; set; }

        // Rate Yaw
        public float RatYawpidff { get; set; }

        public float RatYawpidP { get; set; }

        public float RatYawpidI { get; set; }

        public float RatYawpidD { get; set; }

        public float RatYawTar { get; set; }

        public float RatYawAct { get; set; }
        ...
        //Added by HL below
        public float dose_information { get; set; }
        //Added by HL above
