...
namespace MissionPlanner.GCSViews.ConfigurationView
{
    public partial class ConfigHWCompass : UserControl, IActivate
    {
        ...
        private void BUT_OBmagcalstart_Click(object sender, EventArgs e)
        {
            ...
            mag1_cprobar.Value = 0;
            mag1_cprobar.Text = "1号: 0%";
            horizontalProgressBar2.Value = 0;
            mag2_cprobar.Value = 0;
            mag2_cprobar.Text = "2号: 0%";
            horizontalProgressBar3.Value = 0;
            mag3_cprobar.Value = 0;
            mag3_cprobar.Text = "3号: 0%";
        ...
        private void timer1_Tick(object sender, EventArgs e)
        {
                ...
                foreach (var item in status)
                {
                    try
                    {
                        if (item.Key == 0)
                        {
                            ...
                            mag1_cprobar.Value = item.Value;
                            mag1_cprobar.Text = "1号: " + item.Value.ToString() + "%";
                        }
                        if (item.Key == 1)
                        {
                            horizontalProgressBar2.Value = item.Value;
                            mag2_cprobar.Value = item.Value;
                            mag2_cprobar.Text = "2号: " + item.Value.ToString() + "%";
                        }
                        if (item.Key == 2)
                        {
                            horizontalProgressBar3.Value = item.Value;
                            mag3_cprobar.Value = item.Value;
                            mag3_cprobar.Text = "3号: " + item.Value.ToString() + "%";
                        }
                    }
                    catch { }

                    message += "指南针" + item.Key + ": 校准进度 " + item.Value.ToString() + "% ";
                    compasscount++;
                }
            ...
            lock (mrep)
            {
                ...
                {
                    lbl_obmagresult.AppendText("指南针" + item.compass_id + ": x:" + item.ofs_x.ToString("0.0") + " y:" + item.ofs_y.ToString("0.0") + " z:" +
                                             item.ofs_z.ToString("0.0") + " 精度值:" + item.fitness.ToString("0.0") + " " + (MAVLink.MAG_CAL_STATUS)item.cal_status + "\n");
                    ...
                    try
                    {
                        if (item.compass_id == 0)
                        {
                            ...
                            mag1_cprobar.Value = 100;
                            mag1_cprobar.Text = "1号: 完成";
                        }
                        if (item.compass_id == 1)
                        {
                            horizontalProgressBar2.Value = 100;
                            mag2_cprobar.Value = 100;
                            mag2_cprobar.Text = "2号: 完成";
                        }
                        if (item.compass_id == 2)
                        {
                            horizontalProgressBar3.Value = 100;
                            mag3_cprobar.Value = 100;
                            mag3_cprobar.Text = "3号: 完成";
                        }
                    }
                    catch { }
                ...
                if (compasscount == completecount && compasscount != 0)
                {
                    ...
                    timer1.Stop();
                    MessageBox.Show("恭喜校准成功，重启飞控后生效！", "校准成功", MessageBoxButtons.OK, MessageBoxIcon.Information);
                }
            }
        }
