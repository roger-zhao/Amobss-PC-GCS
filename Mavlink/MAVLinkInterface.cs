...
namespace MissionPlanner
{
    public class MAVLinkInterface : MAVLink, IDisposable
    {
        ...
        // need same as  flightplanner
        enum mission_item_info
        {
            LABEL_AUTOTO_WP = 0x20,
            LABEL_RESUME_WP = 0x40,
        };
        ...
        public bool setParam(byte sysid, byte compid, string paramname, double value, bool force = false)
        {
            ...
            while (true)
            {
                if (!(start.AddMilliseconds(700) > DateTime.Now))
                {
                    ...
                    giveComport = false;
                    // throw new TimeoutException("配置数据失败，请尝试重新连接飞控");
                    MessageBox.Show("配置数据失败，请尝试重新连接飞控", "错误", MessageBoxButtons.OKCancel, MessageBoxIcon.Error);
                    return false;
                }
    ...
    Strings.Gotparam + "请稍候...");
        ...
        public float GetParam(byte sysid, byte compid, string name = "", short index = -1, bool requireresponce = true)
        {
            ...
            while (true)
            {
                if (!(start.AddMilliseconds(700) > DateTime.Now))
                {
                    ...
                    giveComport = false;
                    // throw new TimeoutException("刷新飞控数据超时") --tmp delete, need to confirm;
                    // MessageBox.Show("刷新飞控数据超时", "错误", MessageBoxButtons.OKCancel, MessageBoxIcon.Error);
                    return 0;
                }
        ...
        public Locationwp getHomePosition()
        {
            ...
            while (true)
            {
                if (!(start.AddMilliseconds(700) > DateTime.Now))
                {
                    ...
                    //return (byte)int.Parse(param["WP_TOTAL"].ToString());
                    // throw new TimeoutException("刷新飞控数据超时") --tmp delete, need to confirm;
                    MessageBox.Show("获取飞控GPS位置超时", "错误", MessageBoxButtons.OKCancel, MessageBoxIcon.Error);
        ...
        public Locationwp getWP(ushort index)
        {
            ...
            while (true)
            {
                if (!(start.AddMilliseconds(3500) > DateTime.Now)) // apm times out after 5000ms
                {
                    ...
                    giveComport = false;
                    MessageBox.Show("获取航点超时", "错误", MessageBoxButtons.OKCancel, MessageBoxIcon.Error);
                    return new Locationwp();
                }
                ...
                if (buffer.Length > 5)
                {
                    if (buffer.msgid == (byte) MAVLINK_MSG_ID.MISSION_ITEM)
                    {
                        ...
                        loc.options = wp.frame;
#if false
                        if ((wp.item_info.Equals((ushort)mission_info.e_autoTO_WP))) // autoTO_wp
                        {
                            loc.options |= (byte)mission_item_info.LABEL_AUTOTO_WP; // get mission item_info bit1 as label of autoTO_wp: for options: 0x20
                        }
                        else if ((wp.item_info.Equals((ushort)mission_info.e_resume_WP))) // resume_WP
                        {
                            loc.options |= (byte)mission_item_info.LABEL_RESUME_WP; // get mission item_info bit1 as label of autoTO_wp: for options: 0x30
                        }
#endif
                        // (loc.options & 0x1) == 0 ? MAV_FRAME.GLOBAL : MAV_FRAME.GLOBAL_RELATIVE_ALT;
        ...
        public void setWPTotal(ushort wp_total)
        {
            ...
            while (true)
            {
                if (!(start.AddMilliseconds(700) > DateTime.Now))
                {
                    ...
                    giveComport = false;
                    MessageBox.Show("下发航点总数超时", "错误", MessageBoxButtons.OKCancel, MessageBoxIcon.Error);
                    return;
                }
                MAVLinkMessage buffer = readPacket();
                if (buffer.Length > 9)
            ...
            byte autocontinue = 1, bool use_int = false)
        {
            ...
            if (use_int)
            {
                ...
                // AB ZhaoYJ for distinguish autoTO WP
                if((loc.options & (byte)mission_item_info.LABEL_AUTOTO_WP) != 0x0)
                {
                    req.item_info = (ushort)mission_info.e_autoTO_WP;
                }
                else if ((loc.options & (byte)mission_item_info.LABEL_RESUME_WP) != 0x0)
                {
                    req.item_info = (ushort)mission_info.e_resume_WP;
                }
#endif
        ...
        public MAV_MISSION_RESULT setWP(mavlink_mission_item_t req)
        {
            ...
            while (true)
            {
                if (!(start.AddMilliseconds(400) > DateTime.Now))
                {
                    ...
                    giveComport = false;
                    MessageBox.Show("航点下发超时", "错误", MessageBoxButtons.OKCancel, MessageBoxIcon.Error);
        ...
        public int getRequestedWPNo()
        {
            ...
            while (true)
            {
                ...
                {
                    giveComport = false;
                    MessageBox.Show("航点获取超时", "错误", MessageBoxButtons.OKCancel, MessageBoxIcon.Error);
                    return 0;
                }
                MAVLinkMessage buffer = readPacket();
                if (buffer.Length > 5)
        ...
        public MAVLinkMessage readPacket()
        {
            ...
            try
            {
                if ((message.header == 'U' || message.header == 0xfe || message.header == 0xfd) && buffer.Length >= message.payloadlength)
                {
                    ...
                    if (msgid == (byte)MAVLink.MAVLINK_MSG_ID.COMMAND_LONG) // status text
                    {
                        mavlink_command_long_t cmd_long = message.ToStructure<mavlink_command_long_t>();
                    ...
                    if (msgid == (byte)MAVLink.MAVLINK_MSG_ID.STATUSTEXT) // status text
                    {
                        ...
                        /// start
                        logdata = zhHansSpecialMsg(logdata);
...
// flush on heartbeat - 1 seconds
        ...
        void getWPsfromstream(ref MAVLinkMessage buffer, byte sysid, byte compid)
        {
            ...
            if (buffer.msgid == (byte) MAVLINK_MSG_ID.PARAM_VALUE)
            {
                ...
                log.Info("getParam: " + st + " : " + MAVlist[sysid, compid].param[st]);
        ...
        public PointLatLngAlt getFencePoint(int no, ref int total)
        {
            ...
            while (true)
            {
                if (!(start.AddMilliseconds(700) > DateTime.Now))
                {
                    ...
                    giveComport = false;
                    MessageBox.Show("刷新地理围栏数据超时", "错误", MessageBoxButtons.OKCancel, MessageBoxIcon.Error);
        ...
        public FileStream GetLog(ushort no)
        {
            ...
            while (true)
            {
                if (!(start.AddMilliseconds(3000) > DateTime.Now))
                {
                    ...
                    giveComport = false;
                    MessageBox.Show("获取日志超时", "错误", MessageBoxButtons.OKCancel, MessageBoxIcon.Error);
                    return null;
                }
        ...
        public mavlink_log_entry_t GetLogEntry(ushort startno = 0, ushort endno = ushort.MaxValue)
        {
            ...
            while (true)
            {
                if (!(start.AddMilliseconds(2000) > DateTime.Now))
                {
                    ...
                    giveComport = false;
                    MessageBox.Show("获取日志条目超时", "错误", MessageBoxButtons.OKCancel, MessageBoxIcon.Error);
                    return new mavlink_log_entry_t();
                }
        ...
        public PointLatLngAlt getRallyPoint(int no, ref int total)
        {
            ...
            while (true)
            {
                if (!(start.AddMilliseconds(700) > DateTime.Now))
                {
                    ...
                    giveComport = false;
                    MessageBox.Show("获取备降点超时", "错误", MessageBoxButtons.OKCancel, MessageBoxIcon.Error);
                    return new PointLatLngAlt();
                }
        ...
        /// for: translate some status txt to Mandrin
        /// TODO: make sure it's stable
        /// start
        public string zhHansSpecialMsg(string msg)
        {
            Level lvl_log = Level.Info;
            string newMsg = "";
            if (msg.ToLower().Contains("farring"))
            {
                newMsg = "远智飞控FA1";
            }
            else if (msg.ToLower().Contains("frame"))
            {
                if (msg.ToLower().Contains("quad"))
                {
                    newMsg = "已连接四轴飞行器";
                    this.MAV.cs.copter_type = "四轴飞行器";
                }
                else if (msg.ToLower().Contains("hexa"))
                {
                    newMsg = "已连接六轴飞行器";
                    this.MAV.cs.copter_type = "六轴飞行器";
                }
                else if (msg.ToLower().Contains("octa"))
                {
                    newMsg = "已连接八轴飞行器";
                    this.MAV.cs.copter_type = "八轴飞行器";
                }
                else
                {
                    newMsg = msg;
            ...
            else if (msg.ToLower().Contains("expired date"))
            {
                newMsg = "抱歉，使用期限已到，请找供应商延长使用日期";
                lvl_log = Level.Critical;
            }
            else if (msg.ToLower().Contains("arm: mode not armable"))
            {
                newMsg = "当前模式无法解锁";
                lvl_log = Level.Error;

            }
            else if (msg.ToLower().Contains("low battery"))
            {
                newMsg = "电池电量不足";
                lvl_log = Level.Warning;

            }
            else if (msg.ToLower().Contains(" mode failed"))
            {
                if (msg.ToLower().Contains(" 0 mode"))
                {
                    newMsg = "手动自稳模式切换失败";
                    lvl_log = Level.Error;

                }
                else if (msg.ToLower().Contains(" 2 mode"))
                {
                    newMsg = "手动增稳模式切换失败";
                    lvl_log = Level.Error;
                }
                else if (msg.ToLower().Contains(" 3 mode"))
                {
                    newMsg = "自动航线模式切换失败";
                    lvl_log = Level.Error;
                }
                else if (msg.ToLower().Contains(" 4 mode"))
                {
                    newMsg = "定宽模式切换失败";
                    lvl_log = Level.Error;
                }
                else if (msg.ToLower().Contains(" 5 mode"))
                {
                    newMsg = "自动悬停模式切换失败";
                    lvl_log = Level.Error;
                }
                else if (msg.ToLower().Contains(" 6 mode"))
                {
                    newMsg = "自动返航模式切换失败";
                    lvl_log = Level.Error;
                }
            ...
            else if (msg.ToLower().Contains("no takeoff wp"))
            {
                newMsg = "尚未添加一键起飞功能，请添加后再试；或者在手动模式下将飞机升高到2米以上，再切换自动航线模式";
                MainV2.speechEngine.SpeakAsync(newMsg);
                lvl_log = Level.Error;
            }
            else if (msg.ToLower().Contains("ekf compass variance"))
            {
                newMsg = "严重磁干扰，请尝试重新校准指南针或者改变起飞地点";
                MainV2.speechEngine.SpeakAsync(newMsg);
                lvl_log = Level.Error;
            }
            else if (msg.ToLower().Contains("need 3d fix") || msg.ToLower().Contains("gps speed error"))
            {
                newMsg = "卫星搜索中";
                lvl_log = Level.Warning;
            }
            else if (msg.ToLower().Contains("locate copter alarm"))
            {
                newMsg = "解锁 请使用 内八字";
                lvl_log = Level.Warning;
            }
            else if (msg.ToLower().Contains("esc calibration done, please reboot"))
            {
                newMsg = "电调校准 完成 请重启 飞行器";
            }
            // prearm
            else if (msg.ToLower().Contains("prearm: rc "))
            {
                newMsg = "解锁前 请首先校准 遥控器";
                lvl_log = Level.Warning;
            }
            else if (msg.ToLower().Contains("prearm: barometer not healthy"))
            {
                newMsg = "气压计 异常 无法解锁";
                lvl_log = Level.Warning;
            }
            else if (msg.ToLower().Contains("prearm: compass not calibrated"))
            {
                newMsg = "指南针 未校准";
                lvl_log = Level.Error;
            }
            else if (msg.ToLower().Contains("prearm: check mag field"))
            {
                newMsg = "磁场强度 异常";
                lvl_log = Level.Error;
            }
            else if (msg.ToLower().Contains("prearm: accels not calibrated"))
            {
                newMsg = "请先校准 加速度计";
                lvl_log = Level.Warning;
            }
            else if (msg.ToLower().Contains("prearm: accelerometers not healthy"))
            {
                newMsg = "加速度计 初始化";
            }
            else if (msg.ToLower().Contains("prearm: gyros not healthy"))
            {
                newMsg = "陀螺仪 初始化";
                lvl_log = Level.Error;
            }
            else if (msg.ToLower().Contains("prearm: check board voltage"))
            {
                newMsg = "飞控板 电源异常";
                lvl_log = Level.Error;
            }
            else if (msg.ToLower().Contains("prearm: check battery"))
            {
                newMsg = "请检查 动力电源";
                lvl_log = Level.Error;
            }
            else if (msg.ToLower().Contains("prearm: waiting for nav checks"))
            {
                newMsg = "请稍候 飞控 初始化中";
            }
            else if (msg.ToLower().Contains("prearm: high gps hdop") || (msg.ToLower().Contains("prearm:") && (msg.ToLower().Contains("gps"))))
            {
                newMsg = "卫星锁定中";
                lvl_log = Level.Warning;
            }
            else if (msg.ToLower().Contains("prearm: battery failsafe"))
            {
                newMsg = "电池电量不足";
                lvl_log = Level.Warning;
            }
            else if (msg.ToLower().Contains("ekf warmup"))
            {
                newMsg = "请稍候 飞控 初始化中";
            }
            // accel calibration
            else if (msg.ToLower().Contains("level"))
            {
                newMsg = "请 水平放置 飞行器，静待1到2秒后，按任意键完成 水平面校准";
            }
            else if (msg.ToLower().Contains("left side"))
            {
                newMsg = "请将 机头 左面朝下放置 飞行器，静待1到2秒后，按任意键完成 左面校准";
            }
            else if (msg.ToLower().Contains("right side"))
            {
                newMsg = "请将机头 右面朝下放置 飞行器，静待1到2秒后，按任意键完成 右面校准";
            }
            else if (msg.ToLower().Contains("nose down"))
            {
                newMsg = "请将机头 朝下放置 飞行器，静待1到2秒后，按任意键完成 朝下校准";
            }
            else if (msg.ToLower().Contains("nose up"))
            {
                newMsg = "请将机头 朝上放置 飞行器，静待1到2秒后，按任意键完成 朝上校准";
            }
            else if (msg.ToLower().Contains("its back"))
            {
                newMsg = "请 倒置 飞行器 （也就是正面朝下），静待1到2秒后，按任意键完成 背面校准";
            }
            else if (msg.ToLower().Contains("successful")
                || msg.ToLower().Contains("calibrated"))
            {
                newMsg = "校准成功";
            }
            else if (msg.ToLower().Contains("fail") && !msg.ToLower().Contains("failsafe") && !msg.ToLower().Contains("failed"))
            {
                newMsg = "抱歉，校准失败，请尝试重新校准";
                lvl_log = Level.Error;
            }
            else if (msg.ToLower().Contains("flight plan received"))
            {
                newMsg = "航线下发成功";
            }
            else if (msg.ToLower().Contains("flight plan update rejected"))
            {
                newMsg = "航线更新失败";
                lvl_log = Level.Error;
            }
            else if ((msg.ToLower().Contains("starting calibration"))
                || (msg.ToLower().Contains("waiting for init"))
                || (msg.ToLower().Contains("barometer calibration complete"))
                || (msg.ToLower().Contains("accelrange"))
                || (msg.ToLower().Contains("offsets"))
                || (msg.ToLower().Contains("scaling"))
                || (msg.ToLower().Contains("ge comp"))
                || (msg.ToLower().Contains("calibrating barometer"))
                || msg.Contains("EKF primary changed")
                || msg.Contains("SMARTRTL")
                || msg.Contains("EKF2 IMU")
                || msg.Contains("u-blox")
            ...
            // 365B cali
            else if (msg.ToLower().Contains("365b cali:"))
            {
                newMsg = msg.Replace("365b cali", "航向仪校准第") ;
                newMsg = newMsg + "步 完成";
            }
            // mission interrupt
            else if (msg.ToLower().Contains("mission interrupt"))
            {
                newMsg = "航线中断";
                lvl_log = Level.Info;
            }
            else if (msg.ToLower().Contains("resume index:"))
            {
                newMsg = msg.Replace("resume index:", "断点续航于");
                newMsg = newMsg + "号航点";
                lvl_log = Level.Info;

            }
            else if (msg.ToLower().Contains("reached command"))
            {
                newMsg = msg.Replace("Reached command #", "到达航点");
            }
            else if (msg.Contains("AB Points clear"))
            {
                newMsg = "航线已被清除";
                lvl_log = Level.Info;
            }
            else if (msg.Contains("rc7 is in A point"))
            {
                newMsg = "悬停模式下，遥控器通道7请回中位";
            }
            else if (msg.ToLower().Contains("a points set"))
            {
                newMsg = "A点设置成功";
            }
            else if (msg.ToLower().Contains("b points set"))
            {
                newMsg = "B点设置成功";
            }
            else if (msg.ToLower().Contains("caution: radio lost") || msg.ToLower().Contains("no receiver"))
            {
                newMsg = "遥控器信号丢失";
                lvl_log = Level.Error;
            }
            else if (msg.Contains("PreArm: Throttle below Failsafe"))
            {
                newMsg = "遥控器未打开";
                lvl_log = Level.Warning;
            }
            else if (msg.Contains("PreArm: 3D Accel calibration needed"))
            {
                newMsg = "IMU需要校准";
                lvl_log = Level.Warning;
            }
            ...
            // message from FC
            if(newMsg != "")
                MainV2.lblog.Log(lvl_log, newMsg);
