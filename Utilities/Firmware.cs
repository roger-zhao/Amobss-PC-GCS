...
namespace MissionPlanner.Utilities
{
    public class Firmware
    {
        ...
        public bool update(string comport, software temp, string historyhash)
        {
            ...
            var ans = UploadFlash(Settings.GetUserDataDirectory() + @"firmware.hex", "", "", 0);
...
//VR boards have no tone alarm
        ...
        public bool UploadFlash(string filename, string type, string frame, int sn_num)
        {
             // return UploadFarring(filename, type, frame, sn_num);
             return UploadFarringG3(filename);
        ...
        public bool UploadFarring(string filename, string vtype, string vframe, int sn_num)
        {
            ...
            // scp transfer FC version
            var sftp_inst = new SftpClient(farring_IP, 22, user_name, farring_pwd);
            sftp_inst.Connect();
            if (sftp_inst.IsConnected)
            {
                // update dtb @2017-10-12
                // update SWI2C dtb @2017-12-13
                var fileStream = new FileStream("./SWI2C.dtb", FileMode.Open);
                if (fileStream != null)
                {       
                    sftp_inst.UploadFile(fileStream, "/boot/uboot/dtbs/farring-SWI2C.dtb", null);

                }
                // update app-start @2017-10-12
                fileStream = new FileStream("./app-start.sh", FileMode.Open);
                if (fileStream != null)
                {
                    sftp_inst.UploadFile(fileStream, "/lib/systemd/system/app-start.sh", null);
                ...
                if (fileStream != null)
                {
                    ...
                    //
                    dest_fn = filename.Contains("飞控版本") ? Path.GetFileNameWithoutExtension(filename).Replace("飞控版本", "FW-") + ".tar" : Path.GetFileNameWithoutExtension(filename);
                    sftp_inst.UploadFile(fileStream, "/var/APM/" + dest_fn, null);
            ...
            using (var client = new SshClient(farring_IP, user_name, farring_pwd))
            {
                ...
                while (!MainV2.connectOK)
                {
                    if (try_conn1++ > 5)
                    {
                        CustomMessageBox.Show("抱歉，飞控版本升级失败，请尝试重新连接飞控后，再次升级飞控[10]!");
                        return false;
                    }
                    MainV2.instance.connectFC("TCP");
                }

                // read frame current for validating
                int orig_frame = Convert.ToInt32(MainV2.comPort.GetParam("FRAME"));

                // validation
                // validate copter type
                bool copter_type_keep = false;
                bool copter_frame_keep = false;

                if (!vtype.Equals(MainV2.comPort.MAV.cs.copter_type))
                {
                    // cancel
                    if (CustomMessageBox.Show("请确认：飞行器类型变更!!! \n\n从<" + MainV2.comPort.MAV.cs.copter_type + "> 升级为 <" + vtype + ">？", "确认",
    MessageBoxButtons.OKCancel) != DialogResult.OK)
                    {
                        CustomMessageBox.Show("已取消升级飞控软件，请重启飞控!");
                        return false;
                    }
                    copter_type_keep = false;
                }
                else // copter type not change, so notify to keep params original
                {
                    copter_type_keep = true;
                }

                // validate copter frame
                int frame_val = vframe.Equals("十字型") ? 0 : (vframe.Equals("X 字型") ? 1 : -1);
                if (frame_val.Equals(-1))
                {
                    CustomMessageBox.Show("机架类型选择错误，暂时只支持 <十字型> 和 <X 字型> 机架，请重新选择!");
                    return false;
                }
                if (!orig_frame.Equals(frame_val))
                {
                    if (CustomMessageBox.Show("请确认：机架类型变更!!! \n\n从 < " + ((orig_frame.Equals(0)) ? "十字型" : "X 字型") + " > 升级为 < " + vframe + " >？", "确认",
MessageBoxButtons.OKCancel) != DialogResult.OK)
                    {
                        CustomMessageBox.Show("已取消升级飞控软件，请重启飞控!");
                        return false;
                    }
                    copter_frame_keep = false;
                    // CustomMessageBox.Show("请确认：机架类型变更!!! \n\n从<" + ((orig_frame.Equals(0)) ? "十字型" : "X 字型") + "> 升级为 <" + vframe + ">？");
                }
                else
                {
                    copter_frame_keep = true;
                }


                // update dtb @2017-10-12
                client.RunCommand("cp /boot/uboot/dtbs/farring-SWI2C.dtb /boot/uboot/dtbs/am335x-boneblack.dtb");
                client.RunCommand("cp /boot/uboot/dtbs/farring-SWI2C.dtb /boot/uboot/dtbs/am335x-boneblack.dtb; sync; sync");
                // reduce boot time
                // update app-start @2017-10-12
                client.RunCommand("chmod 777 /lib/systemd/system/app-start.sh"); //  cp /boot/uboot/dtbs/farring-gps-en.dtb /boot/uboot/dtbs/am335x-boneblack.dtb");
                client.RunCommand("mv /etc/init.d/console-setup /etc/init.d/console-setup.unuse"); //  cp /boot/uboot/dtbs/farring-gps-en.dtb /boot/uboot/dtbs/am335x-boneblack.dtb");
                client.RunCommand("mv /etc/init.d/keyboard-setup /etc/init.d/keyboard-setup.unuse; sync; sync"); // /etc/init.d/console-setup /etc/init.d/console-setup.unuse"); //  cp /boot/uboot/dtbs/farring-gps-en.dtb /boot/uboot/dtbs/am335x-boneblack.dtb");

                client.RunCommand("cp /var/APM/" + dest_fn + " /root/");
                client.RunCommand("cd /root");
                string cmd = "echo ******************************************************************************" + " >> " + " /var/APM/logs/update_log";
                client.RunCommand(cmd);

                string update_item = DateTime.Now.ToString() + " to " + dest_fn;
                cmd = "echo " + update_item + " >> " + " /var/APM/logs/update_log";
                client.RunCommand(cmd);
                ...
                cmd = "rm -f " + dest_fn;
                client.RunCommand(cmd);
                cmd = "rm -f " + "/var/APM/" + dest_fn;
                client.RunCommand(cmd);
                // cp readme
                cmd = "cat APP-new/readme" + " >> " + " /var/APM/logs/update_log";
                client.RunCommand(cmd);
                cmd = "systemctl stop app.service";
                client.RunCommand(cmd);
                client.RunCommand(cmd);
                cmd = "rm -rf APP";
                client.RunCommand(cmd);
                // map to new FW according to type & frame
                cmd = "chmod u+x APP-new/*; mv APP-new APP;sync;sync";
                client.RunCommand(cmd);

                string update_sn_cmd;
                switch (vtype)
                {
                    case "四轴飞行器":
                        // update app
                        cmd = "ln -sf APP/app_quad app";
                        client.RunCommand(cmd);

                        // update SN
                        update_sn_cmd = "sed -ir '/type/c " + "type: quad' /root/SN";
                        client.RunCommand(update_sn_cmd);
                        break;
                    case "六轴飞行器":
                        // update app
                        cmd = "ln -sf APP/app_hexa app";
                        client.RunCommand(cmd);

                        // update SN
                        update_sn_cmd = "sed -ir '/type/c " + "type: hexa' /root/SN";
                        client.RunCommand(update_sn_cmd);
                        break;
                    case "八轴飞行器":
                        // update app
                        cmd = "ln -sf APP/app_octa app";
                        client.RunCommand(cmd);

                        // update SN
                        update_sn_cmd = "sed -ir '/type/c " + "type: octa' /root/SN";
                        client.RunCommand(update_sn_cmd);
                        break;
                }

                // update SN ver
                string fw_ver = dest_fn.Substring(dest_fn.IndexOf('V'), 5);
                update_sn_cmd = "sed -ir '/FWVer/c " + "FWVer: '" + fw_ver + " /root/SN";
                client.RunCommand(update_sn_cmd);

                // if copter type & frame not change, keep params as before, 
                // else reset all params to default
                // TODO: need to different details issue
                if (copter_type_keep && copter_frame_keep)
                {
                    cmd = "sync;systemctl restart app.service";
                }
                else
                {
                    cmd = "sync;rm -f /var/APM/ArduCopter.stg;systemctl restart app.service";
                }

                client.RunCommand(cmd);
                MainV2.connectOK = false;
                ...
                while (!MainV2.connectOK)
                {
                    if(try_conn++ > 5)
                    {
                        CustomMessageBox.Show("抱歉，飞控版本升级失败，请尝试重新连接飞控后，再次升级飞控[11]!");
                        return false;
                    }
                    MainV2.instance.connectFC("AUTO");
                }
                ...
                try
                {
                    ...
                    {
                        MainV2.comPort.setParam("FRAME", (int)frame_val);
                        Thread.Sleep(500);

                        if (Convert.ToInt32(MainV2.comPort.GetParam("FRAME")).Equals(frame_val))
                        {
                            MainV2.comPort.setParam("VEHICEL_ID", sn_num);
                            Thread.Sleep(500);
                            int sn_num_str = Convert.ToInt32(MainV2.comPort.GetParam("VEHICEL_ID"));
                            if (sn_num_str.Equals(sn_num))
                            {
                                // update SN
                                string sn_frame = frame_val.Equals(0) ? "+" : (frame_val.Equals(1) ? "x" : "unkown");

                                update_sn_cmd = "sed -ir '/frame/c " + "frame:'" + sn_frame + " /root/SN";
                                client.RunCommand(update_sn_cmd);
                                break;
                            }
                    ...
                    if(5 == try_times)
                    {
                        CustomMessageBox.Show("抱歉，飞控版本升级失败，请尝试重新连接飞控后，再次升级飞控[12]!");
                    }
                    else
                    {
                        // 重启飞控
                        cmd = "reboot";
                        client.RunCommand(cmd);
                        CustomMessageBox.Show("恭喜，飞控版本升级成功，请等待飞控重新启动!");
                    }

                }
                catch
                {
                    CustomMessageBox.Show("抱歉，飞控版本升级失败，请尝试重新连接飞控后，再次升级飞控[13]!");
                }
        ...
        public bool UploadFarringG3(string filename)
        {
            ...
            if (sftp_inst.IsConnected)
            {
                ...
                if (fileStream != null)
                {
                    ...
                    //
                    dest_fn = filename.Contains("飞控版本") ? Path.GetFileNameWithoutExtension(filename).Replace("飞控版本", "FW-") + ".fc" : Path.GetFileNameWithoutExtension(filename);
                    sftp_inst.UploadFile(fileStream, "/root/" + dest_fn, null);
            ...
            using (var client = new SshClient(farring_IP, user_name, farring_pwd))
            {
                ...
                try { 
                    ...
                    // update
                    cmd = "sh /root/Ver/update_fc.sh";
                    client.RunCommand(cmd);

                    MessageBox.Show("恭喜，飞控版本升级成功，请等待飞控重新启动!");
                }
                catch
                {
                    MessageBox.Show("抱歉，飞控版本升级失败，请尝试重新连接飞控后，再次升级飞控[13]!");
                }
